...
export async function getShops( onError: (error: string) => void) {
    try {
        const url = `${ADMIN_ROUTES.GET_SHOPS}`;
        const token = localStorage.getItem('token')
        const response = await axios.get(url, {
            headers: {
                Authorization: `Bearer ${token}`
...
export async function getCategories( onError: (error: string) => void) {
    try {
        const url = `${ADMIN_ROUTES.GET_CATEGORIES}`;
        const token = localStorage.getItem('token')
        const response = await axios.get(url, {
            headers: {
                Authorization: `Bearer ${token}`
